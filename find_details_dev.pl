...
find_db(Dept,File,Output_type,Output) :-
...
% a b
 	...
 	phrase_from_file_s(string(Codes_cve), "../../Philosophy/connectives.txt"),
...
%trace,
	...
	concat_list(["find_details_db_",File],File1),
	(open_s(File1,write,Stream1),
	write(Stream1,D83),
	close(Stream1)),
	...
	concat_list(["find_details_wn_",File],File2),
...
load_db(Dept,File,[file_or_string,Input_file],WN,DB) :-

% randomly choose a b from a b c

	concat_list([Dept,"/","find_details_wn_",File],File1),

		phrase_from_file_s(string(String00awn), File1),
	...
	concat_list([Dept,"/","find_details_db_",File],File2),

		phrase_from_file_s(string(String00a), File2),
		string_codes(String02b,String00a),
		atom_to_term(String02b,DB,[]),

	(file_or_string=file->
	(	concat_list([Dept,"/","file_fddb"],File3),
		phrase_from_file_s(string(String00a1), File3),
		string_codes(String02b1,String00a1),
		downcase_atom(String02b1,Input_file)
	);
	Input_file=(-))
		,!.
		
			
find_synth(Dept,File,[file_or_string,Input_file]) :-%(Chain2) :-

	load_db(Dept,File,[file_or_string,Input_file],WN,DB),

 	SepandPad="&#@~%`$?-+*^,()|.:;=_/[]<>{}\n\r\s\t\\\"!0123456789", % doesn't have "'" xx

	split_string(Input_file, ".\n\r", ".\n\r", String02cb),
